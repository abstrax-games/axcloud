...
export default async function postRoutes(fastify: FastifyInstance) {
    ...
    // 查看某个完整投稿
    fastify.get<{ Params: { id: number } }>('/:id/full', async (request, reply) => {
        const { id } = request.params;
    ...
    fastify.post<{ Body: PostBody }>('/new', {
        ...
        try {
            ...
            await fastify.logAction(request.user.username, `Create post #${newId}`, ip, `Content: ${snap}`);
        }
        catch (e) {
            console.log(e);
    ...
    // 编辑投稿
    fastify.post<{ Params: { id: number }, Body: PostBody }>('/edit/:id', {
        preHandler: withPrivilegeCheck(PrivilegeChecker.SEND_POST)
    }, async (request, reply) => {
        // 获取IP
        const ip = (request.headers['x-real-ip'] as string) || request.ip;
        ...
        try {
            ...
            post.score = score;
            post.rank = rank;
            post.chooseMethod = chooseMethod;
            post.chooseCourse = chooseCourse;
            post.content = content;
            post.updateTime = new Date();

            if (!PrivilegeChecker.check(userInfo, PrivilegeChecker.MANAGE_POST)) {
                // 如果不是管理员，就要更新available为false
        ...
        try {
            ...
            await fastify.logAction(user, `Edit post #${id}`, ip, `Change To ${snap}`);
        }
        catch (e) {
            console.log(e);
            return { error: true, message: 'Error creating log' };
